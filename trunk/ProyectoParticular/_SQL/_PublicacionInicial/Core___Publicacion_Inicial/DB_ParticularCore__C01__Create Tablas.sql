...
(
    id								INT   IDENTITY(1,1)	NOT NULL
	,UsuarioOriginanteId			INT				NOT NULL	DEFAULT 1	-- El que "pide" el envío, que puede ser distinto de quien la carga.
	,UsuarioDestinatarioId			INT				NOT NULL 	DEFAULT 1	-- id=1 cuando "no hay usuario". / Serivrá para corroborar a quién se lo quisimois manda. No se usa en el envío.
...
(
	...
	,SeDebenEvaluarPermisos	BIT 			NOT NULL	DEFAULT 1 -- En la Validación de Autorizado A.
	,EsUnaConRegistro		BIT 			NOT NULL			  -- Para la Validación de Autorizado A.
...
(
	...
	,Fecha							DATETIME		NOT NULL -- De envío
...
(
	...
	,Observaciones					VARCHAR(1000)	NULL	-- Obs nuestras, posteriormente, con la solución
...
(
	...
	,Fecha									DATETIME		NOT NULL -- De creación de la Nota.
...
(
	id								INT   IDENTITY(1,1) NOT NULL
	,ContextoId						INT				NOT NULL
	--,UsuarioResponsableId			INT				NOT NULL -- SE CREA UNA REL PARA TENER MULTIPLES APROBADORES. (ANTES: No necesariamente será el que apruebe su reserva.)
	,Nombre							VARCHAR(100)	NOT NULL
	,Observaciones					VARCHAR(1000)	
	,Activo							BIT				NOT NULL	DEFAULT 1	
...
-- Prioridad: Define la prioridad sobre otro rol, para la asignación de permisos. Prioridades < -1000 son para las particularidades:
(
    ...
    ,SeMuestraEnAsignacionDePermisos		BIT				NOT NULL	DEFAULT 1		-- Roles A Páginas por parte del Administrador
...
CREATE TABLE SPs
	 ...
	 -------------
	 -- Acotaciones:
		-- Se usa para excluir mirar permisos y para controlar la generación de Emails y Notificaciones.
	 -------------
(
    id										INT   IDENTITY(1,1) NOT NULL
...
(
	...
	-- Los Campos a continuación son para gestionar los SP creados AUTO.
