...
BEGIN TRY
	...
	IF @AuthCookie IS NOT NULL AND @AuthCookie <> '' 
		BEGIN -- Autenticacion con cookie
			...
			SELECT @id = U.id
				,@ContextoId = CON.id
				,@Activo = U.Activo
				,@PassDelUsuario = U.Pass -- No es necesario, pero si para que no rompa una comparación posterior.
			FROM Usuarios U
			...
			WHERE (U.AuthCookie = @AuthCookie)
				...
				--AND (@AuthCookie IS NOT NULL) // Se controla más adelante
				--AND	(U.Activo = 1) // Se controla más adelante
		END
	ELSE
		BEGIN -- Autenticacion con UserNamer + Contexto, y Pwd
			...
			WHERE 
				...
				-- AND (Pass = @Pass) // Se controla más adelante
				-- AND	(U.Activo = 1) // Se controla más adelante
		END	
	...
	SET @VecesDeLoginFallido = COALESCE((SELECT COUNT(id) 
			...
			--11	0	Fallido: @UsuarioQueEjecutaId <> 1	Error en el Login: Usuario inválido para realizar esta tarea.
			...
			--21	1	Fallido: c/Cookie y @UsuarioQueEjecutaId <> 1	Error en el Login: Usuario inválido para realizar esta tarea.
